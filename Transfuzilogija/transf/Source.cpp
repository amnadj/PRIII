...
public:
	...
	void Sort(string sort) {
		...
		while (promjena)
		{
			...
			{
				if (_elementi1[i] > _elementi1[i + 1] && _keyElement == 1 && sort == "ASC") {
					swap(_elementi1[i], _elementi1[i + 1]);
					swap(_elementi2[i], _elementi2[i + 1]);
					promjena = true;
				}

				else if (_elementi1[i] < _elementi1[i + 1] && _keyElement == 1 && sort == "DESC") {
					swap(_elementi1[i], _elementi1[i + 1]);
					swap(_elementi2[i], _elementi2[i + 1]);
					promjena = true;
				}
				else if (_elementi2[i] > _elementi2[i + 1] && _keyElement == 2 && sort == "ASC") {
					swap(_elementi1[i], _elementi1[i + 1]);
					swap(_elementi2[i], _elementi2[i + 1]);
					promjena = true;
				}
				else if (_elementi2[i] < _elementi2[i + 1] && _keyElement == 2 && sort == "DESC") {
					swap(_elementi1[i], _elementi1[i + 1]);
					swap(_elementi2[i], _elementi2[i + 1]);
					promjena = true;
				}
...
public:
	...
	string ToString() {
		...
		s += crt;
		s += "Krvna grupa-> ";
		s += OznakaKrvneGrupeString[_oznaka];
		s+=_rhFaktor;
		s += crt;
		...
		{
			s += OznakaKrvneGrupeString[_donatori[i]._oznaka];
			s += _donatori[i]._rhFaktor;
			s += ", ";
		}
		s += "\n";
		s += "Primaoci-> ";
		...
		{
			s += OznakaKrvneGrupeString[_primaoci[i]._oznaka];
			s+=_primaoci[i]._rhFaktor;
			s += ", ";
		}
	...
	bool operator==(const KrvnaGrupa& k) {
		if(_oznaka == k._oznaka && _rhFaktor==k._rhFaktor)
		return false;
	}
};
...
public:
	...
	KrvnaGrupa getKrvna() const { return _krvnaGrupa; }
	Osoba(const Osoba& o):_krvnaGrupa(o._krvnaGrupa) {
		int vel = strlen(o._imePrezime) + 1;
		_imePrezime = new char[vel];
		strcpy_s(_imePrezime, vel, o._imePrezime);
	}
...
bool ValidanFormat(string telefon) {
	return regex_match(telefon, regex("\d{3}"));
}
...
public:
	...
	void SetDatumPosljednjeg(Datum d){
...
////datum donacije, donator, kolicina
	void AddDonaciju(Datum d,Osoba* o,double kol) {
		bool postojikg = false;
		for (int i = 0; i < _zalihe.GetTrenutno(); i++)
		{
			if (*_zalihe.GetElement1(i) == o->getKrvna()) {
		...
		if (!postojikg)
			_zalihe.AddElement(new KrvnaGrupa(o->getKrvna()), kol);

		Donator* donator = dynamic_cast<Donator*>(o);
		for (int i = 0; i < _donacije.GetTrenutno(); i++)
		{
			if (*_donacije.GetElement1(i) == *o) {
				dynamic_cast<Donator*>(_donacije.GetElement1(i))->SetDatumPosljednjeg(d);
				_donacije.SetElement2(i, _donacije.GetElement2(i) + kol);
			}
		}
		
		_donacije.AddElement(new Donator(*donator), kol);
	}
};
